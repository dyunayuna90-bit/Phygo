...
function renderResultStep(body, cfg){
  body.innerHTML = `
    ...
    <div style="margin-top:24px; background:var(--surface-c); border-radius:var(--r-l); padding:8px 20px; box-shadow: inset 0 2px 8px rgba(0,0,0,0.3);">
      ...
      <div class="result-row" style="border:none; padding-bottom:8px;"><span>Selisih Kesalahan</span><b style="color:${cfg.success?'var(--success)':'var(--error)'}">${cfg.diff}</b></div>
    </div>
  ...
  requestAnimationFrame(()=>{
    ...
    gsap.to('#accBar', {width:fmt(cfg.accuracy,0)+'%', duration:1, ease:'power3.out', delay:.2});
  });
