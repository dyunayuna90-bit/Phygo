...
function l1Ex(body){
  ...
  body.innerHTML = `<span class="example-tag">${svgIcon('speed')} Mari Perhatikan</span><h2>Motor melaju stabil 10 m/s selama 5 detik</h2>${buildRoadStageHTML('#5FE1E2','#063B3C',false)}<p class="step-text">Kecepatannya tetap dari awal sampai akhir. Amati pergerakannya yang halus tanpa hentakan.</p><button class="btn btn-ghost ripple-host btn-sm" id="btnReplay">Putar Ulang Simulasi</button>`;
  ...
  function play(){ 
    ...
    gsap.to([car,sh], { x: tX, duration: 1.8, ease:'none', onUpdate:function(){
      if(this.progress() > 0.1 && this.progress() < 0.9 && Math.random() > 0.6) spawnDustAdv(document.getElementById('roadStage'), 10 + gsap.getProperty(car, 'x') + 10);
      wheels.forEach(w => w.style.transform = `rotate(${this.progress() * 1080}deg)`);
    }, onComplete:()=>{
    ...
    <div style="display:flex; justify-content:center; margin: 24px 0;">
      <div style="position:relative; width:260px; height:140px; background:radial-gradient(ellipse at bottom, #1A2224, transparent); border-radius:130px 130px 0 0; border:2px solid #2A363A; border-bottom:none; display:flex; justify-content:center; align-items:flex-end; padding-bottom:10px; box-shadow:inset 0 10px 20px rgba(0,0,0,0.8);">
        <svg width="240" height="130" viewBox="0 -25 200 125" style="position:absolute; bottom:10px;">
          ...
          <path d="M10 90 A90 90 0 0 1 190 90" fill="none" stroke="var(--primary)" stroke-width="12" stroke-linecap="butt" stroke-dasharray="283" stroke-dashoffset="283" id="gaugeArc" style="transition: stroke-dashoffset 0.1s linear; filter:drop-shadow(0 2px 4px rgba(0,0,0,0.5));"/>
          <g id="needle" style="transform-origin: 100px 90px; transition: transform 0.1s linear;">
            <path d="M96 90 L100 15 L104 90 Z" fill="#fff" filter="drop-shadow(0 2px 4px rgba(0,0,0,0.5))"/>
...
function l1Hasil(body){
  ...
  renderResultStep(body, { level:1, retryStep:4, success:r.success, accuracy:clamp(100-(Math.abs(r.dist-r.S)/r.S)*100,0,100), varName:'v', unit:'m/s', correctAnswer:fmt(r.S/r.T,2),
    given:[['Kecepatan Mesin Disetel', fmt(r.v,1)+' m/s'],['Batas Waktu Eksekusi', fmt(r.T,1)+' dtk']], computed:fmt(r.dist,1)+' m', computedLabel:'Jarak Aktual yang Ditempuh', target:fmt(r.S,1)+' m', targetLabel:'Jarak Target Zona', diff:fmt(Math.abs(r.dist-r.S),1)+' m',
