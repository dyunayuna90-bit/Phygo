...
function l2Ex(body){
  ...
  function play(){ 
    ...
    gsap.to([car,sh], { x: tX_car, duration:2.0, ease:'power2.in', onUpdate:function(){
       ...
       if(p > 0.3 && Math.random() > 0.55) spawnExhaust(document.getElementById('roadStage'), 10 + curX, '#FFB871');
    }, onComplete:()=>{ 
...
function runL2(a, S, T, mToPx){
  ...
  const tl=gsap.timeline({onComplete:()=>{ 
    ...
    hud.classList.add('show'); hud.innerHTML=texi(String.raw`s = \tfrac{1}{2} a t^2 = \tfrac{1}{2} \times ${fmt(a,1)} \times ${fmt(T,1)}^2 = ${fmt(dist,1)}\text{ m}`);
...
function l2Hasil(body){
  ...
  renderResultStep(body, { level:2, retryStep:6, success:r.success, accuracy:clamp(100-(Math.abs(r.dist-r.S)/r.S)*100,0,100), varName:'a', unit:'m/s²', correctAnswer:fmt((2*r.S)/(r.T*r.T),2),
    given:[['Daya Dorong (Akselerasi) Diatur', fmt(r.a,1)+' m/s²'],['Batas Waktu Eksekusi', fmt(r.T,1)+' dtk']], computed:fmt(r.dist,1)+' m', computedLabel:'Jarak Aktual yang Dicapai', target:fmt(r.S,1)+' m', targetLabel:'Jarak Target Zona', diffValue:Math.abs(r.dist-r.S), diffUnit:'m', diffDecimals:1,
