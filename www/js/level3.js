...
function l3Ex(body){
  ...
  buildRuler(h); const b=document.getElementById('ball'), stg=document.getElementById('fallStage'), tail=document.getElementById('mbTail'), tm=document.getElementById('stgTimer');
  ...
  function play(){ 
    ...
    if(tm) tm.textContent = fmt(0,2)+' dtk';
    ...
    gsap.to(b,{
      ...
      onUpdate: function() {
         ...
         if(tm) tm.textContent = fmt(prog*t,2)+' dtk';
         if(prog > 0.2) { 
...
function runL3(pred, H){
  ...
  gsap.to(b, {
    ...
    onComplete: function() {
      ...
      tPts.push({x: cT, y: H});
      gr.ctx.clearRect(0,0,gr.w,gr.h); drawAxes(gr.ctx,gr.w,gr.h);
      const xMEnd = Math.max(cT, pred) * 1.15;
      drawSeries(gr.ctx,gr.w,gr.h,tPts,xMEnd,H*1.05,'#FFB4AB',true);
...
function l3Hasil(body){
  ...
  renderResultStep(body, { level:3, retryStep:6, success:r.success, accuracy:clamp(100-(Math.abs(r.pred-r.cT)/r.cT)*100,0,100), varName:'t', unit:'detik', correctAnswer:fmt(r.cT,2),
    given:[['Elevasi Jatuh (h)', fmt(r.H,1)+' m'],['Intensitas Gravitasi (g)', '9,8 m/s²']], computed:fmt(r.pred,2)+' s', computedLabel:'Tebakan Sensor Durasi', target:fmt(r.cT,2)+' s', targetLabel:'Titik Hantam Aktual', diffValue:Math.abs(r.pred-r.cT), diffUnit:'s', diffDecimals:2,
